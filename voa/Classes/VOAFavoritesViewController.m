...
@implementation VOAFavoritesViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
		...
		UITabBarItem* item = [[UITabBarItem alloc] initWithTabBarSystemItem:UITabBarSystemItemFavorites
																		tag: TT_FAVORITES];
		...
		self.title = NSLocalizedString(@"Favoites",@"");
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated 
{
	...
	{
		VOACategoriesDetailTableViewCell* cell = (VOACategoriesDetailTableViewCell*)[_tableView cellForRowAtIndexPath: 
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if(cell)
	{
		...
		if(item)
			item.isFavorited = [NSNumber numberWithBool:NO];
...
NSComparisonResult dateSortCopy(Item *s1, Item *s2, void *context) 
{
...
- (void)fetch 
{
	...
	{
		NSArray* sortedArray = fetchedResultsController.fetchedObjects;
		sortedArray = [sortedArray sortedArrayUsingFunction:dateSortCopy context:nil];
		[_itemArray addObjectsFromArray:sortedArray];
	}
